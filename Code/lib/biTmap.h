...
unsigned int bit,pos,col,row,line,max_pos;
struct bitmap_t {
	int32_t widt;
	int32_t height;
...
int read_bitmap_from_file(const char* filename, struct bitmap_t* bitmap) {
	...
	// width of a byte row
	if (bitmap->widt % 8) {
		bitmap->rowwidth = ((bitmap->widt/8)+1) * bitmap->depth;
	} else {
		bitmap->rowwidth = (bitmap->widt/8) * bitmap->depth;
	}
	...
	fprintf(stdout, "File '%s' is a %ix%ix%i bitmap\n", filename, bitmap->widt, bitmap->height, bitmap->depth);
	...
	for ( row=0; row<bitmap->height; ++row) {
		...
		if (bottomup) {
			...
			memcpy(bitmap->data+(row*abs(bitmap->widt/8)), buffer, bitmap->rowwidth);
		}
...
void print_bitmap(struct bitmap_t* bitmap) {
	for ( row=0; row<bitmap->height; ++row) {
		...
		for (col = 0; col<bitmap->rowwidth; ++col) {
			print_binary(bitmap->data[row*bitmap->rowwidth+col], col == bitmap->rowwidth -1 ? bitmap->widt % 8 : 8);
		}
		printf("\n");
	}
}
...
int write_bitmap_as_xbm(const char* filename, struct bitmap_t* bitmap, const char* name) {
	...
	for (line = 0; line <= (bitmap->height*bitmap->rowwidth)/12; ++line) {
		fprintf(fd, "\t");

		 max_pos = (line < (bitmap->height*bitmap->rowwidth)/12) ? 12 : (bitmap->height*bitmap->rowwidth) % 12;
